...
TEST_F(FixedMemoryResourceTest, MemoryReuse) {
    ...
    size_t allocated_count1 = memory_resource->get_allocated_count();
    
    memory_resource->deallocate(ptr1, 100);
    size_t allocated_count2 = memory_resource->get_allocated_count();
    
    EXPECT_EQ(allocated_count1 - 1, allocated_count2);
...
// Интеграционный тест переиспользования памяти
TEST(MemoryReuseIntegrationTest, CyclicPushPop) {
    ...
    size_t allocated_before = memory.get_allocated_count();
    size_t free_before = memory.get_free_count();
    ...
    size_t allocated_after_pop = memory.get_allocated_count();
    size_t free_after_pop = memory.get_free_count();
    
    EXPECT_LT(allocated_after_pop, allocated_before);
    EXPECT_GT(free_after_pop, free_before);
    ...
    size_t allocated_after_reuse = memory.get_allocated_count();
    size_t free_after_reuse = memory.get_free_count();
    
    EXPECT_EQ(allocated_after_reuse, allocated_before);
    EXPECT_LT(free_after_reuse, free_after_pop);
}

// Тест категории итератора
